...
export default function BookAppointment() {
  ...
  useEffect(() => {
    axios.get("http://localhost:8082/doctor/getAllDoctors").then((res) => {
      setAllDoctors(res.data);
    });

    axios.get("http://localhost:8086/timing/getAllTiming").then((res) => {
      setAllTime(res.data);
    });
  }, []);
  axios
    .get(
      "http://localhost:8086/appointment/appointmentExists/" + state.user.email
    )
    .then((res) => {
      if (res.data == true) {
        setBook(true);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8081/patient/getpat/" + state.user.email)
      .then((res) => {
        setPatient(res.data);
      })
      .catch((err) => {});
                  ...
                  >
                    ...
                    <div className="card-footer" style={{ textAlign: "right" }}>
                      <select
                        ...
                        disabled={doctor.availability == "No" || isBooked}
                      >
                                ...
                                if (res.data <= 3) {
                                  axios
                                    ...
                                    .then(function (response) {
                                      ...
                                      axios.get("http://localhost:8086/timing/"+timings).then(res=>{
                                        axios.post("http://localhost:8087/sendEmail",{"toEmail":patient.email,"subject":"Appointment Registration Successful","body":"Your Appointment is Registered and your approximate time is "+res.data.appointmenttime+" for Dr."+doctor.doctor_name+" ("+doctor.doctor_id+")"}).catch(function (error) {
                                          console.log(error);
                                        });
                                      })
