...
export default function SignupPage() {
  ...
  const [patient_name, setFullName] = useState("");
  const [age, setAge] = useState("");
  const [email, setEmail] = useState("");
  const [patient_phone_no, setPhno] = useState("");
      ...
      .then((result) => {
        if (result == false) {
          fetch("http://localhost:8081/patient/add", {
            ...
            axios.post("http://localhost:8087/sendEmail",{"toEmail":email,"subject":"Confirmation Mail","body":"Welcome to Online Medical Portal...."}).catch(function (error) {
              console.log(error);
            });
          });
          navigate("/login");
